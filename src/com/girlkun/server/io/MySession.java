        ...
        if (this.player != null) {
            ...
            try {
                ...
                if (player != null) {
                    ...
                    Logger.warning("Đăng Nhập Thành Công Player " + this.player.name + "\nIP:"+player.getSession().getIP() + ": " + (System.currentTimeMillis() - st) + " ms\n");
                   ChatGlobalService1.gI().chat(player,"Top 1 sức mạnh " + player.name + " vừa đăng nhập ");
                }
            } catch (Exception e) {
                if (player != null) {
                    player.dispose();
