...
//        Service.gI().sendThongBaoOK(session, "Vui lòng đăng ký tài khoản tại trang chủ!");
    ...
    public void sendInfo(MySession session) {
        ...
        if (TaskService.gI().getIdTask(player) == ConstTask.TASK_0_0) {
            NpcService.gI().createTutorial(player, -1,
                    "Chào mừng " + player.name + " đến với Server Trùng Sinh\n"
        ...
        if (player.istrain && MapService.gI().isMapTrainOff(player, player.zone.map.mapId)) {
         ...
         if (player.inventory.itemsBag.get(7).isNotNullItem()) {
            new Thread(() -> {
                try {
                    Thread.sleep(1000);
                    Service.getInstance().sendFoott(player, (short) player.inventory.itemsBag.get(7).template.id);
                } catch (Exception e) {
                }
            }).start();
        }
    ...
    private void sendThongBaoServer(Player player) {
        Service.gI().sendThongBaoFromAdmin(player, "|7| Chào Mừng " + player.name + " Đến Với Server Trùng Sinh\b|2| Chúc Anh Em Chơi Game Vui Vẻ...\nThời gian offline của bạn là " + player.timeoff + " phút");
    ItemTimeService.gI().sendTextTime(player, (byte) KHI_GASS, "Mỗi ngày nhận ngẫu nhiên 10 đá nâng cấp tại Bà Hạt Mít ", 30);
