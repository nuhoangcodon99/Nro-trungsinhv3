...
public class PlayerDAO {
    ...
    public static boolean createNewPlayer(int userId, String name, byte gender, int hair) {
        try {
            ...
            GirlkunDB.executeUpdate("insert into player"
                    ...
                    + "data_black_ball, data_side_task, data_card, bill_data,data_item_time_sieu_cap,info_achievement, data_offtrain) "
                    ...
                    itemTime, task, mabuEgg, charms, skills, skillsShortcut, petData, dataBlackBall, dataSideTask, data_card, bill_data, itemTimeSC, info_achive, dataoff);
            ...
            try {
                ...
                if (player.isDie()) {
                    ...
                    if (MapService.gI().isMapDoanhTrai(mapId) ||MapService.gI().isMapBlackBallWar(mapId)
                            || MapService.gI().isMapBanDoKhoBau(mapId) || MapService.gI().isMapConDuongRanDoc(mapId) || MapService.gI().isMapMaBu(mapId) || MapService.gI().isMapKhiGaHuyDiet(mapId)) {
                        mapId = player.gender + 21;
                        x = 300;
                        y = 336;
                    }
 ...
 //data danh hiệu
                ...
                String query = " update player set info_achievement =?,data_item_time_sieu_cap = ?, head = ?, have_tennis_space_ship = ?,"
                        ...
                        + " skills_shortcut = ?, pointPvp=?, NguHanhSonPoint=?, point_gapthu=?, point_cauca=?, point_vip=?, capboss=?, capvithu=?, diemdanh=?, data_offtrain =?, data_card=?,bill_data =? where id = ?";
                GirlkunDB.executeUpdate(query,
                        ...
                        billEgg,
                        player.id);
                Logger.log(Logger.CYAN_BACKGROUND, "Player: " + player.name + " đã rời khỏi Sever! \n");
            } catch (Exception e) {
