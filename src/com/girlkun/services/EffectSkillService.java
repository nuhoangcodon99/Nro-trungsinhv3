...
import com.girlkun.models.player.Charms;
...
//Cải trang Drabura Frost
    ...
    public void SetHoaCarot(Player player, long lastTimeHoaCarot, int timeHoaCarot){
        ...
        player.charms.tdBatTu = lastTimeHoaCarot;
        sendEffectPlayer(player, player, TURN_ON_EFFECT, (byte) CAROT);
    }
    public void removeCarot(Player player){
     ...
     public void Setxuong(Player player, long lastTimeHoaXuong, int timeHoaXuong){
        player.effectSkill.lastTimeHoaXuong = lastTimeHoaXuong;
        player.effectSkill.timeXuong = timeHoaXuong;
        player.effectSkill.isxuong = true;
        player.nPoint.isxuong = true;
         player.nPoint.tlHutHp = 5;
        player.nPoint.giamsd  = 0;
        
    }
    public void removeXuong(Player player){
        player.effectSkill.isxuong = false;
        Service.gI().Send_Caitrang(player);
         Service.gI().Send_Caitrang(player);
    ...
    public void setMabu(Player player, long lastTimemabu, int timemabu) {
        player.effectSkill.lastTimemabu = lastTimemabu;
        player.effectSkill.timemabu = timemabu;
        player.effectSkill.ismabu = true;
        player.effectSkill.countPem1hp = 0;
         sendEffectPlayer(player, player, TURN_OFF_EFFECT, CAROT);
    }

    //player trở lại thành người
    public void removeMabu(Player player) {
        player.effectSkill.ismabu = false;
        Service.gI().Send_Caitrang(player);
         sendEffectPlayer(player, player, TURN_OFF_EFFECT, CAROT);
    }
    ...
    public void sendMobToSocola(Player player, Mob mob, int timeSocola) {
        ...
        try {
            ...
            msg.writer().writeShort(16416); //icon socola
    ...
    //**************************************************************************
    public void setxuong(Player player) {
        int timeMonkey = SkillUtil.getTimeMonkey(player.playerSkill.skillSelect.point);
        if (player.setClothes.xuong == 0) {
            timeMonkey *= 0;
        }
        player.effectSkill.isxuong = true;
        player.effectSkill.timeXuong = timeMonkey;
        player.effectSkill.lastTimeHoaXuong= System.currentTimeMillis();
        player.nPoint.xuong(player.nPoint.hp * 2);
    }

    public void XuongDown(Player player) {
        player.effectSkill.isxuong = false;
    ...
    public void sendEffectCharge(Player player) {
        ...
        try {
            ...
            msg.writer().writeShort(skill.skillId);
            Service.gI().sendMessAllPlayerInMap(player, msg);
            Service.gI().chat(player, "Tái Tạo Năng Lượng...");
