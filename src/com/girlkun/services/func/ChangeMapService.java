...
import com.girlkun.models.boss.list_boss.Tau77;
...
public class ChangeMapService {
   ...
   private static final byte EFFECT_GO_TO_CDRD = 3;
...
//        zoneJoin = checkMapCanJoin(pl, zoneJoin);
        if (zoneJoin != null) {
            ...
            if (pl.playerTask != null && !pl.isBoss && !pl.isPet && !pl.isNewPet && pl.playerTask.taskMain.id == 5 && pl.playerTask.taskMain.index == 6) {
                try {
                    if (pl.zone.map.mapId == 47) {
                        new Tau77(BossID.TAU77, BossesData.Tau_77, pl.zone, pl.location.x - 10, pl.location.y);
            ...
            if (!pl.isBoss && !pl.isPet && !pl.isNewPet && !pl.isfake) {
        ...
        try {
            for (Mob mob : player.zone.mobs) {
                ...
                if (mob.effectSkill.isSocola) {
                    ...
                    msg.writer().writeShort(16414);//b5
                    player.sendMessage(msg);
                    msg.cleanup();
                }
