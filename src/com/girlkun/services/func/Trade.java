...
public class Trade {
    ...
    if (player1.getSession().actived && player2.getSession().actived) {
        ...
        try {
            ...
            Logger.logException(DataGame.class, IO);
        }
    } else {
        Service.getInstance().sendThongBao(player1, "Không thể thực hiện giao dịch vì một trong hai người chơi không được kích hoạt.");
        Service.getInstance().sendThongBao(player2, "Không thể thực hiện giao dịch vì một trong hai người chơi không được kích hoạt.");
    }
}
...
//        System.out.println("quantity: " + quantity);
        if (pl.getSession().actived) {
                ...
                if (index == -1) {
                    ...
                    if (pl.equals(this.player1)) {
                        ...
                        if(index>=0 && index<itemsBag2.size()){
                            ...
                            System.out.println("loi index"+index);
                        }
                    }
                    ...
                    if (isItemCannotTran(item)) {
                        ...
                        if (quantity > 1000) {
                            int n = quantity / 1000;
                            int left = quantity % 1000;
                            for (int i = 0; i < n; i++) {
                                Item itemTrade = ItemService.gI().copyItem(item);
                                itemTrade.quantity = 1000;
                                if (pl.equals(this.player1)) {
            ...
            Service.getInstance().sendThongBaoFromAdmin(pl,
                    "|5|VUI LÒNG IB VỚI ADMIN ĐỂ KÍCH HOẠT TÀI KHOẢN\n|5|ĐỂ MỞ KHÓA TÍNH NĂNG GIAO DỊCH");
            removeItemTrade(pl, index);
        }
    }
    ...
    public void cancelTrade() {
        String notifiText = "Giao dịch bị hủy bỏ số lượng k được vượt qua 999";
