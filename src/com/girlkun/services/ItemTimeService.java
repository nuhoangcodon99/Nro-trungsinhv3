...
public class ItemTimeService {
    ...
    public static ItemTimeService gI() {
     ...
     public void sendTextDoanhTrai2(Player player) {
        if (player.clan != null && !player.clan.haveGoneDoanhTrai2
                && player.clan.timeOpenDoanhTrai2 != 0) {
            int secondPassed = (int) ((System.currentTimeMillis() - player.clan.timeOpenDoanhTrai2) / 1000);
            int secondsLeft = (Zombie.TIME_DOANH_TRAI / 1000) - secondPassed;
            sendTextTime(player, DOANH_TRAI2, "Động Mị Nương", secondsLeft);
        }
    }
      public void removeTextDoanhTrai2(Player player) {
        removeTextTime(player, DOANH_TRAI2);
    }

    //gửi cho client
    public void sendAllItemTime(Player player) {
        sendTextDoanhTrai(player);
        sendTextDoanhTrai2(player);
        ...
        if (player.itemTime.isUseCuaRangMe) {
            sendItemTime(player, 8060, (int) ((TIME_ITEM - (System.currentTimeMillis() - player.itemTime.lastTimeCuaRangMe)) / 1000));
        }
        ...
        if (player.itemTime.isUseBinhCanx10) {//binh cần x10
         ...
         if (player.itemTime.isUseBanhTet) {
            sendItemTime(player, 25510,
                    (int) ((TIME_ITEM45P - (System.currentTimeMillis() - player.itemTime.lastTimeBanhTet)) / 1000));
        }

        if (player.itemTime.isUseBanhChung) {
            sendItemTime(player, 25509,
                    (int) ((TIME_ITEM45P - (System.currentTimeMillis() - player.itemTime.lastTimeBanhChung)) / 1000));
        }
