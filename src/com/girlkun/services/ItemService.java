...
public class ItemService {
    ...
    public void OpenItem1361(Player player, Item itemUse) {
        ...
        Util.isTrue(30, 100);
        Item bkt = ItemService.gI().createNewItem((short) 457);
        bkt.quantity += 0;
        InventoryServiceNew.gI().addItemBag(player, bkt);
        Service.gI().sendThongBao(player, "Bạn Nhận Được 1 điểm Câu Cá và 1 " + bkt.template.name);
    }

    public void OpenItem1362(Player player, Item itemUse) {
        Util.isTrue(70, 100);
        player.point_cauca += 3;
        Util.isTrue(30, 100);
        Item bktt = ItemService.gI().createNewItem((short) 457);
        bktt.quantity += 1;
        InventoryServiceNew.gI().addItemBag(player, bktt);
        Service.gI().sendThongBao(player, "Bạn Nhận Được 1 điểm Câu Cá và 2 " + bktt.template.name);
    }
    ...
    public void OpenDTL(Player player, int gender, int itemtype) {

        int[][] items = {{555, 556, 562, 563, 561}, {557, 558, 564, 565, 561}, {559, 560, 566, 567, 561}}; //td, namec,xd
        Item item = randomCS_DTL(items[player.gender][itemtype], gender);
    ...
    public Item randomCS_DTL(int itemId, int gender) {
        ...
        if (ao.contains(itemId)) {
            it.itemOptions.add(new Item.ItemOption(47, Util.highlightsItem(gender == 2, new Random().nextInt(501) + 1300)));
        }
        if (quan.contains(itemId)) {
            it.itemOptions.add(new Item.ItemOption(22, Util.highlightsItem(gender == 0, new Random().nextInt(11) + 45)));
        }
        if (gang.contains(itemId)) {
            it.itemOptions.add(new Item.ItemOption(0, Util.highlightsItem(gender == 2, new Random().nextInt(1001) + 3500)));
        }
        if (giay.contains(itemId)) {
            it.itemOptions.add(new Item.ItemOption(23, Util.highlightsItem(gender == 1, new Random().nextInt(11) + 35)));
        }
        if (nhd == itemId) {
            it.itemOptions.add(new Item.ItemOption(14, new Random().nextInt(2) + 15));
        }
