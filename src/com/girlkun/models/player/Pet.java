...
public class Pet extends Player {
    ...
    public void joinMapMaster() {
        if (status != GOHOME && status != FUSION && !isDie()) {
            this.location.x = this.master.location.x + Util.nextInt(-10, 10);
            this.location.y = this.master.location.y;
            if (!this.isBoss) { 
                ChangeMapService.gI().goToMap(this, this.master.zone);
            ...
            switch (status) {
                case FOLLOW:
                 ...
                 case PROTECT:
                    ...
                    List<Player> players = this.zone.getBosses();
                    if (players.size() > 0) {
                    for (Player pl : players) {
                        if (pl!= null && pl.isBoss)
                        {
                ...
                case ATTACK:
                    ...
                    List<Player> players1 = this.zone.getBosses();
                    if (players1.size() > 0) {
