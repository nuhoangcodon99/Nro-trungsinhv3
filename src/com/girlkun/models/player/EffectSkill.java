...
public class EffectSkill {
    ...
    public int timeBang;
     ...
     //Cải trang xương Khô
    public boolean isxuong;
    public long lastTimeHoaXuong;
    public int timeXuong;
...
//        }
        ...
        if (isBlindDCTT && (Util.canDoWithTime(lastTimeBlindDCTT, timeBlindDCTT))) {
         ...
         if (isBang && (Util.canDoWithTime(lastTimeHoaBang, 15000)) ) {
              EffectSkillService.gI().removeBang(this.player);
        }
        if (isDa && (Util.canDoWithTime(lastTimeHoaDa, 15000)) ) {
              EffectSkillService.gI().removeDa(this.player);
        }
        if (isxuong && (Util.canDoWithTime(lastTimeHoaXuong, 1800000)) ) {
              EffectSkillService.gI().removeXuong(this.player);
        }
         if (isCarot && (Util.canDoWithTime(lastTimeHoaCarot, 300000)) ) {
              EffectSkillService.gI().removeCarot(this.player);
        }
          if (istm && (Util.canDoWithTime(lastTimetm, timeThoiMien))) {
            EffectSkillService.gI().removeThoiMien(this.player);
        }
          if (ismabu && (Util.canDoWithTime(lastTimemabu, 30000)) ) {
              EffectSkillService.gI().removeMabu(this.player);
        }
