...
import com.girlkun.models.map.Zone;
import com.girlkun.services.ChatGlobalService1;
...
protected Player playerTarger1;
    protected short tilepst;
    
    protected Player playerkill;
     protected Zone bosses;
    ...
    public Player getPlayerAttack() {
        if (this.playerTarger != null && (this.playerTarger.isDie() || !this.zone.equals(this.playerTarger.zone))) {
         ...
         if (this.id == BossID.SON_TINH) {
                for (Player pl : this.zone.bosses) {
                    if (!(pl == null || pl.isDie()) && (pl.id == BossID.THUY_TINH) && Util.canDoWithTime(this.lastTimeTargetPlayer, this.timeTargetPlayer)) {
                        this.playerTarger = pl;
                        this.lastTimeTargetPlayer = System.currentTimeMillis();
                        this.timeTargetPlayer = Util.nextInt(1, 500);
                        break;
                    }
                }
            } else if (this.id == BossID.THUY_TINH) {
                for (Player pl : this.zone.bosses) {
                    if (!(pl == null || pl.isDie()) && (pl.id == BossID.SON_TINH) && Util.canDoWithTime(this.lastTimeTargetPlayer, this.timeTargetPlayer)) {
                        this.playerTarger = pl;
                        this.lastTimeTargetPlayer = System.currentTimeMillis();
                        this.timeTargetPlayer = Util.nextInt(300, 500);
                        break;
                    }}}
...
private void createBossAppearTogether() {
    ...
    protected void notifyJoinMap() {
        ...
        ServerNotify.gI().notify("BOSS " + this.name + " vừa xuất hiện tại " + this.zone.map.mapName +" (" + this.zone.map.mapId + ")" + "");
    }
...
//                Logger.logException(Boss.class, ex);
    ...
    public void checkPlayerDie(Player player) {
        if (player.isDie()) {
            this.chat("Chừa nha "+ player.name);
        }
    }
    ...
    public void die(Player plKill) {
        if (plKill != null) {
            ...
            ChatGlobalService1.gI().chat(plKill,"Quá đỉnh " + plKill.name + " vừa tiêu diệt được " + this.name + " mọi người đều ngưỡng mộ");
        }
       this.changeStatus(BossStatus.DIE);
        if (this.id == BossID.SON_TINH) {
            for (Player pl : this.zone.bosses){
                if (pl.id == BossID.THUY_TINH) {
                    Boss ttt = (Boss) pl;
                    ttt.changeStatus(BossStatus.LEAVE_MAP);
                    break;
                }
            }
        } else if (this.id == BossID.THUY_TINH) {
            for (Player pl : this.zone.bosses) {
                if (pl.id == BossID.SON_TINH) {
                    Boss ttt = (Boss) pl;
                    ttt.changeStatus(BossStatus.LEAVE_MAP);
                    break;
                }
    ...
    public void wakeupAnotherBossWhenAppear() {
        ...
        if (!MapService.gI().isMapMaBu(this.zone.map.mapId) && MapService.gI().isMapBlackBallWar(this.zone.map.mapId)) {
            System.out.println("BOSS " + this.name + " : " + this.zone.map.mapName  + "(" + this.zone.map.mapId + ")");
        }
        if (MapService.gI().isMapMabu13h(this.zone.map.mapId)
