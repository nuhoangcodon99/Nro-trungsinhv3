        ...
        try {
            if ((!player.isPet && player.getSession().actived && player.setClothes.setDHD == 5)) {
                ...
                if (Util.isTrue(5, 100)) {
                    ...
                    Service.gI().sendThongBao(player, "Bạn Nhận Được" + i.template.name);
                }
            }
...
//        }
        ...
        if (this.zone.map.mapId >= 53 && this.zone.map.mapId <= 62) {
            if (Util.isTrue(100, 100)) {
                int[] itemDos = new int[]{76};
                int randomDo = new Random().nextInt(itemDos.length);
                if (Util.isTrue(50, 100)) {
                    for (int i = 0; i < player.zone.map.mapWidth; i += 20) {
                        Service.gI().dropItemMap(this.zone, Util.useItem(zone, itemDos[randomDo], Util.nextInt(20000, 50000), i, this.location.y, player.id));
                    }
                }
            }
        }
